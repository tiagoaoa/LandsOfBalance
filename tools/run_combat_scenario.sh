...
if [ "$SCENARIO" = "PLAY" ] || [ "$SCENARIO" = "ARCHER" ] || [ "$SCENARIO" = "COOP" ] \
        || [ "$SCENARIO" = "WATCH" ]; then
    TIMEOUT_SEC="${2:-3600}"
elif [ "$SCENARIO" = "SKEL" ]; then
    # SKEL waits out a full skeleton revive (a minute) after killing one.
    TIMEOUT_SEC="${2:-140}"
else
    TIMEOUT_SEC="${2:-75}"
fi
...
case "$SCENARIO" in
    A|B|GRASS|PROMO|LOCKON|DODGE|PARRY|BACKSTAB|ESTUS|SOULS|MOVE|COMBO|PLAY|ARROW|ARCHER|RIVER|DRAGON|COOP|COOPSIM|WATCH|SKEL|POSTER|BOWSIM|AIMTEST|REVIVE|MOBSIM|PALSIM|BLOCKSIM|ANIMSIM|GEARSIM|DARKSIM|NOFF) ;;
    *) echo "Scenario must be A, B, GRASS, PROMO, LOCKON, DODGE, PARRY, BACKSTAB, ESTUS, SOULS, MOVE, COMBO, PLAY, ARROW, ARCHER, RIVER, DRAGON, COOP, COOPSIM, WATCH, SKEL, POSTER, BOWSIM, AIMTEST, REVIVE, MOBSIM, PALSIM, BLOCKSIM, ANIMSIM, GEARSIM, DARKSIM, or NOFF (got: $SCENARIO)"; exit 1 ;;
esac
